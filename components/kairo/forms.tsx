...
function GeneralEntityForm({editor,state,busy,onSave,onCancel}:EntityFormProps){
  ...
  return <form onSubmit={submit} className="entity-form">
    ...
    {(editor.kind==="reading"||editor.kind==="ride")&&<><Field label={tr("Vehicle","Transporto priemonė")}><Pick label={tr("Vehicle","Transporto priemonė")} value={wheelId} onChange={selectVehicle} options={vehicleSelectOptions(state,language).map(option=>({...option,disabled:!canRecordWithWheel(state.wheel.find(w=>w.id===option.value))&&!hasArchivedRecord(state,option.value,id)}))}/></Field><Field label={tr("Date and time","Data ir laikas")} hint={`${tr("Device time zone","Įrenginio laiko juosta")}: ${Intl.DateTimeFormat().resolvedOptions().timeZone}.`}><DateTimeInput aria-label={tr("Date and time","Data ir laikas")} required value={at} onValueChange={setAt}/></Field></>}
