...
import {
  ...
  type Entity,type Gear,type Kind,type Maintenance,type Reading,type Ride,type State,type Trip,
...
export function EntityForm({editor,state,busy,onSave,onCancel}:{editor:Editor;state:State;busy:boolean;onSave:(request:FormSubmission)=>Promise<void>;onCancel:()=>void}){
  ...
  const [wheelId,setWheelId]=useState(entity&&"wheelId"in entity?entity.wheelId:linkedReading?.wheelId??state.wheel[0]?.id??"");
