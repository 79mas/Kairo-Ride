...
import {effectiveWheelStatus,ltWheelStatusLabels,vehicleReminder} from "@/lib/kairo/vehicle-status";
...
export type Editor={kind:EditableKind;entity?:Entity;reading?:Reading;parents:string[];readingParents?:string[];tripId?:string;namespace:string};
...
function GeneralEntityForm({editor,state,busy,onSave,onCancel}:EntityFormProps){
  ...
  const linkedInterval=linkedReading?state.wheel.flatMap(wheel=>wheelStats(wheel,state.reading).intervals).find(interval=>interval.reading.id===linkedReading.id):undefined;
  ...
  const [wheelId,setWheelId]=useState(entity&&"wheelId"in entity?entity.wheelId:linkedReading?.wheelId??state.wheel.find(canRecordWithWheel)?.id??"");
  const [at,setAt]=useState(localDateTime(entity&&"at"in entity?entity.at:linkedReading?.at));
  const [distance,setDistance]=useState(rideEntity?.distanceKm!==null&&rideEntity?.distanceKm!==undefined?String(rideEntity.distanceKm):linkedInterval?.distance!==null&&linkedInterval?.distance!==undefined?String(linkedInterval.distance):"");
  ...
  function submit(e:FormEvent){e.preventDefault();try{
    ...
    const recordAt=unchangedAt?originalAt!:new Date(at).toISOString();
    if(editor.kind==="wheel")value={id,name,baselineKm:number(baseline),baselineDate,color,notes,status:vehicleStatus,statusNote:statusNote.trim()};
      ...
      value={id,name,wheelId,at:recordAt,distanceKm:distance.trim()?number(distance):null,tripId:selectedTripId,notes,localDate:unchangedAt&&rideEntity?.localDate?rideEntity.localDate:at.slice(0,10),timeZone:unchangedAt&&rideEntity?.timeZone?rideEntity.timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone};
      ...
      validateRecordTarget(state,value as Ride);
  ...
  return <form onSubmit={submit} className="entity-form">
    ...
    {(editor.kind==="reading"||editor.kind==="ride")&&<><Field label={tr("Vehicle","Transporto priemonė")}><Pick label={tr("Vehicle","Transporto priemonė")} value={wheelId} onChange={selectVehicle} options={state.wheel.map(w=>({value:w.id,label:`${w.name} · ${vehicleLabel(effectiveWheelStatus(w,state))}`,disabled:!canRecordWithWheel(w)&&!hasArchivedRecord(state,w.id,id)}))}/></Field><Field label={tr("Date and time","Data ir laikas")} hint={`${tr("Device time zone","Įrenginio laiko juosta")}: ${Intl.DateTimeFormat().resolvedOptions().timeZone}.`}><Input aria-label={tr("Date and time","Data ir laikas")} type="datetime-local" required value={at} onChange={e=>setAt(e.target.value)}/></Field></>}
    ...
    {editor.kind==="ride"&&<><div className="form-grid"><Field label={tr("Odometer after ride, km","Odometras po važiavimo, km")} hint={state.wheel.find(w=>w.id===wheelId)?`${tr("Latest known","Paskutinis žinomas")}: ${formatKm(wheelStats(state.wheel.find(w=>w.id===wheelId)!,state.reading).odometerKm,locale)} km`:undefined}><Input aria-label={tr("Odometer after ride","Odometras po važiavimo")} inputMode="decimal" value={odometer} onChange={e=>setOdometer(e.target.value)} placeholder={tr("Optional","Neprivaloma")}/></Field><Field label={tr("Ride distance, km","Važiavimo atstumas, km")} hint={tr("Optional. If an odometer is entered, charts use its calculated interval.","Neprivaloma. Įvedus odometrą, grafikai naudoja apskaičiuotą jo intervalą.")}><Input aria-label={tr("Ride distance","Važiavimo atstumas")} inputMode="decimal" value={distance} onChange={e=>setDistance(e.target.value)} placeholder={tr("Optional","Neprivaloma")}/></Field></div><label className="check-card trip-toggle"><input type="checkbox" checked={addAsTrip} onChange={e=>{const checked=e.target.checked;setAddAsTrip(checked);if(checked&&tripId!=="new"&&!state.trip.some(item=>item.id===tripId))setTripId(state.trip[0]?.id??"new");}}/><span>{tr("Add as trip","Pridėti kaip kelionę")}</span></label>{addAsTrip&&<div className="trip-link-fields"><Field label={tr("Trip","Kelionė")}><Pick value={tripId} onChange={setTripId} label={tr("Trip","Kelionė")} options={[{value:"new",label:tr("Create a new trip","Sukurti naują kelionę")},...state.trip.map(t=>({value:t.id,label:t.name}))]}/></Field>{tripId==="new"&&<Field label={tr("New trip name","Naujos kelionės pavadinimas")}><Input aria-label={tr("New trip name","Naujos kelionės pavadinimas")} required maxLength={160} value={newTripName} onChange={e=>setNewTripName(e.target.value)} placeholder={tr("e.g. Baltic coast weekend","Pvz., savaitgalis pajūryje")}/></Field>}</div>}{outside&&<p className="inline-warning"><TriangleAlert/>{tr("The ride date is outside the trip dates.","Važiavimo data nepatenka į kelionės intervalą.")}</p>}</>}
