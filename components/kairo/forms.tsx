...
export type FormSubmission={value:Entity;reading?:Reading|null;readingId?:string;newTrip?:Trip;files?:File[]};
...
export type EntityFormProps={editor:Editor;state:State;busy:boolean;onSave:(request:FormSubmission)=>Promise<void>;onCancel:()=>void;onDirtyChange?:(dirty:boolean)=>void};
...
function GeneralEntityForm({editor,state,busy,onSave,onCancel,onDirtyChange}:EntityFormProps){
  ...
  const initialDuration=rideEntity?.durationMinutes;
  const [rideHours,setRideHours]=useState(initialDuration===undefined?"":String(Math.floor(initialDuration/60)));
  const [rideMinutes,setRideMinutes]=useState(initialDuration===undefined?"":String(initialDuration%60));
  const [odometerReason,setOdometerReason]=useState(linkedReading?.odometerExceptionReason??"");
  const [confirmOdometerException,setConfirmOdometerException]=useState(!!linkedReading?.odometerExceptionReason);
  const distancePreview=recordDistancePreview(state,wheelId??"",linkedReading?.id??id,previewAt,odometer,linkedReading?.sourceOrder,confirmOdometerException?odometerReason:undefined);
  ...
  function submit(e:FormEvent){e.preventDefault();try{
    ...
    if(editor.kind==="wheel")value={id,name,baselineKm:number(baseline),baselineDate,color,notes,status:vehicleStatus,statusNote:statusNote.trim()};
    ...
    else if(editor.kind==="reading"){
      if(distancePreview?.requiresReason&&(!confirmOdometerException||odometerReason.trim().length<3))throw new Error(tr("Confirm why this odometer decrease or reset is intentional.","Patvirtink, kodėl šis odometro sumažėjimas ar atstatymas yra tyčinis."));
      value={id,wheelId,at:recordAt,odometerKm:number(odometer),notes,...(entity&&"sourceOrder"in entity?{sourceOrder:entity.sourceOrder}:{}),...(confirmOdometerException&&odometerReason.trim()?{odometerExceptionReason:odometerReason.trim()}:{})};
    }
    else {
      ...
      if(rideHours.trim()||rideMinutes.trim()){
        const hours=Number(rideHours||0),minutes=Number(rideMinutes||0);
        if(!Number.isInteger(hours)||hours<0||!Number.isInteger(minutes)||minutes<0||minutes>59||hours*60+minutes<=0)throw new Error(tr("Enter a valid time on wheel (hours and 0–59 minutes).","Įvesk tinkamą laiką ant rato (valandas ir 0–59 minutes)."));
      ...
      if(distancePreview?.requiresReason&&(!confirmOdometerException||odometerReason.trim().length<3))throw new Error(tr("Confirm why this odometer decrease or reset is intentional.","Patvirtink, kodėl šis odometro sumažėjimas ar atstatymas yra tyčinis."));
      const reading=odometer.trim()?{id:readingId,wheelId,at:recordAt,odometerKm:number(odometer),notes,...(linkedReading?.sourceOrder!==undefined?{sourceOrder:linkedReading.sourceOrder}:{}),...(confirmOdometerException&&odometerReason.trim()?{odometerExceptionReason:odometerReason.trim()}:{})}:null;
      ...
      request={value,reading,readingId,newTrip,files:addAsTrip?files:[]};
  ...
  return <form onSubmit={submit} onChange={()=>onDirtyChange?.(true)} className="entity-form">
    ...
    {editor.kind==="trip"&&<PendingFiles files={files} onChange={next=>{setFiles(next);onDirtyChange?.(true);}} disabled={busy}/>} 
