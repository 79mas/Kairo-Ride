...
import {ArrowRight,Backpack,BarChart3,Check,CircleHelp,Cloud,CloudOff,FolderOpen,HardDrive,Home,LoaderCircle,Mountain,Paperclip,Pencil,Plus,Route,Settings,ShieldCheck,TriangleAlert,Trash2,WifiOff,Wrench} from "lucide-react";
...
import {databaseRestorations,migrateRideFiles} from "@/lib/kairo/archive";
...
export default function KairoApp(){
  ...
  const refresh=useCallback(async()=>{const ns=namespace.current;let next=await loadWorkspace(ns);const migration=await migrateRideFiles(next.operations,"migration-v207");if(migration.length){await mergeOperations(ns,migration,false);next=await loadWorkspace(ns);}if(ns===namespace.current)setWorkspace(next);},[]);
  useEffect(()=>{
    let mounted=true;
  ...
  async function remove(){
    ...
    try{
      ...
      await commitChanges(t.namespace,changes);await refresh();setPendingDelete(null);setEditor(null);
  ...
  async function saveTripDraft(draft:TripDraft){
    ...
    await refresh();toast.success(tr("Trip saved.","Kelionė išsaugota."));
  }
  ...
  return <div className="kairo-app"><Toaster position="top-center" richColors theme="dark"/><a className="skip-link" href="#main-content">{tr("Skip to content","Pereiti prie turinio")}</a>
    ...
    <Tabs value={view} onValueChange={v=>changeView(v as View)} className="app-tabs"><div className="navigation"><TabsList variant="line" className="main-tabs" aria-label={tr("Main areas","Pagrindinės sritys")}>{nav.map(n=><TabsTrigger value={n.id} key={n.id} aria-label={n.label} title={n.label}><n.icon/><span>{n.label}</span></TabsTrigger>)}</TabsList><span className="nav-caption">{tr("Ride beyond limits.","Riedėk be ribų.")}</span></div><main id="main-content" className="app-main">
      ...
      <MainViews key={profile?.namespace??"local"} state={state} actions={actions} setView={changeView} openStorage={()=>setDriveOpen(true)}/>
    </main></Tabs><EarthProgress state={state} goalId={selectedGoal}/><footer className="app-footer"><span>Kairo Ride <small>2.0.8</small></span><span><ShieldCheck/>{tr("No ads or tracking. Your data stays yours.","Be reklamos ir sekimo. Duomenys lieka tavo.")}</span><Button variant="ghost" size="sm" onClick={()=>setDriveOpen(true)}><CircleHelp/>{tr("Settings","Nustatymai")}</Button></footer>

    <Dialog open={!!editor} onOpenChange={open=>{if(!open&&!busy)setEditor(null);}}><DialogContent className="editor-dialog"><DialogHeader><DialogTitle>{editor?.entity||editor?.reading?tr("Edit","Redaguoti"):tr("New","Naujas įrašas")}: {editor&&(language==="lt"?ltTitles[editor.kind]:titles[editor.kind]).toLowerCase()}</DialogTitle><DialogDescription>{editor?.kind==="ride"?tr("Enter the complete odometer; distance is calculated automatically. A name is optional. You can also create or select a trip.","Įvesk visą odometrą; atstumas skaičiuojamas automatiškai. Pavadinimas neprivalomas. Čia pat galima sukurti arba pasirinkti kelionę."):editor?.kind==="reading"?tr("Enter the complete odometer value, not the ride distance.","Įrašyk visą odometro reikšmę, ne važiavimo atstumą."):editor?.kind==="trip"?tr("A trip combines dates, rides and shared files.","Kelionę sudaro datos, važiavimai ir bendri failai."):editor?.kind==="maintenance"?tr("Set a date, odometer target, or both. Insurance requires an expiry date.","Nurodyk datą, odometro reikšmę arba abu. Draudimui būtina galiojimo data."):tr("Changes are saved only after you press Save.","Pakeitimai išsaugomi tik paspaudus Išsaugoti.")}</DialogDescription></DialogHeader>{editor&&<EntityForm key={`${editor.kind}:${editor.entity?.id??editor.reading?.id??"new"}`} editor={editor} state={state} busy={busy} onSave={save} onCancel={()=>setEditor(null)}/ >}{editor&&(editor.entity||editor.reading)&&<Button className="editor-delete" variant="destructive" disabled={busy} onClick={()=>editor.entity?askDelete(editor.kind,editor.entity,editor.reading):editor.reading&&askDelete("reading",editor.reading)}><Trash2/>{tr("Delete","Pašalinti")}</Button>}{editor?.kind==="trip"&&editor.entity&&<section className="detail-section"><h3>{tr("Trip files","Kelionės failai")}</h3><FileListView files={state.attachment.filter(a=>a.ownerKind==="trip"&&a.ownerId===editor.entity?.id)} localIds={localIds} onDownload={localDownload} onDelete={a=>askDelete("attachment",a)} editable/></section>}</DialogContent></Dialog>

    {activeTrip&&<TripDialog key={(profile?.namespace??"local")+":"+activeTrip.id+":"+Boolean(detail?.edit)} trip={activeTrip} state={state} namespace={profile?.namespace??"local"} initialEdit={detail?.edit} actions={actions} localIds={localIds} onDownload={localDownload} onSave={saveTripDraft} onClose={()=>setDetail(null)}/>}
