...
import {backup,entityKey,formatDate,formatKm,parseBackup,project,today,uuid,validateDelete,validateEdit,type Attachment,type Conflict,type Entity,type Goal,type Kind,type Maintenance,type Reading,type Ride} from "@/lib/kairo/domain";
...
export default function KairoApp(){
  ...
  const actions:ViewActions={openEditor,openRide,askDelete,setDetail,addGoal};
  ...
  return <div className="kairo-app"><Toaster position="top-center" richColors theme="dark"/><a className="skip-link" href="#main-content">{tr("Skip to content","Pereiti prie turinio")}</a>
    ...
    </main></Tabs><EarthProgress state={state}/><footer className="app-footer"><span>Kairo Ride <small>2.0.6</small></span><span><ShieldCheck/>{tr("No ads or tracking. Your data stays yours.","Be reklamos ir sekimo. Duomenys lieka tavo.")}</span><Button variant="ghost" size="sm" onClick={()=>setDriveOpen(true)}><CircleHelp/>{tr("Settings","Nustatymai")}</Button></footer>
