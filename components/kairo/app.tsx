...
export default function KairoApp(){
  ...
  const runSync=useCallback(async(manual:boolean):Promise<SyncOutcome>=>{
    ...
    try{
      ...
      if(result.fileIssues.length){
        const first=result.fileIssues[0],message=result.fileIssues.length===1
          ?tr(`Records synchronized, but “${first.name}” could not be copied to Drive. ${first.message}`,`Įrašai sinchronizuoti, bet failo „${first.name}“ nepavyko nukopijuoti į Drive. ${first.message}`)
          :tr(`Records synchronized, but ${result.fileIssues.length} original files could not be copied to Drive. First: “${first.name}”. ${first.message}`,`Įrašai sinchronizuoti, bet ${result.fileIssues.length} originalių failų nepavyko nukopijuoti į Drive. Pirmas: „${first.name}“. ${first.message}`);
    ...
    catch(e){if(client.current!==api||namespace.current!==ns)return "inactive";const outcome=syncFailureKind(e),message=friendlyError(e);setSyncIssue({message,retry:outcome==="retry"});setConnection(api.connected);void recordError(ns,"DRIVE.SYNC_FAILED",e,{module:"drive",action:"sync",retryable:outcome==="retry"}).catch(()=>{});if(manual)toast.error(message,{duration:8000});await refresh().catch(()=>{});return outcome;}
  ...
  }catch(e){void recordError(form.namespace,"RECORD.SAVE_FAILED",e,{module:"records",action:`save.${form.kind}`}).catch(()=>{});toast.error(friendlyError(e));}finally{saving.current=false;setBusy(false);}}
  async function remove(){
    ...
    }catch(e){void recordError(t.namespace,"RECORD.ARCHIVE_FAILED",e,{module:"records",action:`archive.${t.kind}`}).catch(()=>{});toast.error(friendlyError(e));}finally{setBusy(false);}
  }
  async function activate(next:{client:DriveClient;profile:Profile},copy:boolean){
    ...
    }catch(e){next.client.disconnect();setCandidate(null);void recordError(next.profile.namespace,"ACCOUNT.ACTIVATE_FAILED",e,{module:"account",action:"switch"}).catch(()=>{});toast.error(friendlyError(e));}finally{setConnecting(false);}
  }
  ...
  const pendingFiles=workspace.blobs.filter(blob=>state.attachment.some(a=>a.id===blob.attachmentId&&!a.driveId&&!a.archived)).length;
