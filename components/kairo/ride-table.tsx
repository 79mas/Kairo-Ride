...
import {ArrowDown,ArrowUp,ChevronLeft,ChevronRight,Paperclip,Pencil,TriangleAlert} from "lucide-react";
...
export function RideTable({state,actions}:{state:State;actions:ViewActions}){
  ...
  const entries=useMemo(()=>sortRideEntries(rideEntries(state).filter(entry=>vehicle==="all"||entry.wheelId===vehicle),state,key,direction),[state,key,direction,vehicle]);
  ...
  const columns=(Object.keys(labels) as RideSortKey[]).filter(column=>column!=="files");
  ...
  return <section className="ride-ledger" aria-label={tr("Ride records","Važiavimų įrašai")}>
    <div className="ride-ledger-toolbar"><Pick label={tr("Filter rides by vehicle","Filtruoti važiavimus pagal priemonę")} value={vehicle} onChange={setVehicle} options={[{value:"all",label:tr("All vehicles","Visos priemonės")},...vehicleSelectOptions(state,language)]}/>
    ...
    <div ref={viewport} id="ride-ledger-viewport" className="ride-ledger-viewport" tabIndex={0} role="region" aria-label={tr("Scrollable ride table","Slenkama važiavimų lentelė")} aria-describedby="ride-ledger-help">
      <Table className="ride-ledger-table"><TableCaption className="sr-only">{tr("Rides with estimated kilometres per day. Empty optional columns are hidden on small screens.","Važiavimai su kilometrų per dieną įverčiu. Tušti neprivalomi stulpeliai mažame ekrane paslepiami.")}</TableCaption>
        ...
        <TableBody>{entries.map(entry=>{
          ...
          const open=()=>entry.ride?actions.setDetail({kind:"ride",id:entry.ride.id}):actions.setDetail({kind:"reading",id:entry.reading!.id});
          ...
          return <TableRow key={entry.key} onClick={open} className={entry.warning?"warning-row":undefined}>{columns.map(column=><TableCell key={column} className={columnClass(column)}>{values[column]}</TableCell>)}<TableCell><div className="row-actions"><Button variant="ghost" size="icon" onClick={event=>{event.stopPropagation();actions.openRide(entry.ride,entry.reading);}} aria-label={tr("Edit ride","Redaguoti važiavimą")}><Pencil/></Button></div></TableCell></TableRow>;
        })}</TableBody>
      </Table>
    </div>
  </section>;
