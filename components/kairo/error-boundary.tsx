...
import {diagnosticReport,diagnosticText,recordDiagnostic,shareDiagnosticReport} from "@/lib/kairo/diagnostics";
...
export function CriticalRecovery({error,incidentId}:{error:Error;incidentId?:string}){
  const {tr}=useI18n(),[details,setDetails]=useState(false),[message,setMessage]=useState("");
  async function namespace(){return (await metaGet<Profile>("activeProfile"))?.namespace??"local";}
  async function exportData(){try{const ns=await namespace(),workspace=await loadWorkspace(ns);triggerDownload(new Blob([JSON.stringify(backup(workspace.operations),null,2)],{type:"application/json"}),`Kairo-Ride-${APP_VERSION}-critical-recovery-${new Date().toISOString().replace(/[:.]/g,"-")}.json`);setMessage(tr("Recovery backup downloaded.","Atkūrimo kopija atsisiųsta."));}catch(reason){setMessage(reason instanceof Error?reason.message:String(reason));}}
  async function send(){try{const report=await diagnosticReport(await namespace());const result=await shareDiagnosticReport(report);setMessage(result==="shared"?tr("Share action completed; confirm and send it to the developer.","Bendrinimo veiksmas baigtas; patvirtink ir išsiųsk kūrėjui."):tr("Report downloaded; attach it to the opened email draft.","Ataskaita atsisiųsta; pridėk ją prie atverto laiško."));}catch(reason){setMessage(reason instanceof Error?reason.message:String(reason));}}
  return <main className="startup critical-recovery"><TriangleAlert/><h1>{tr("Kairo Ride encountered a critical error","Kairo Ride įvyko kritinė klaida")}</h1><p>{tr("Your saved records were not intentionally cleared. Export a local recovery copy before clearing browser data or reinstalling the PWA.","Išsaugoti įrašai nebuvo tyčia išvalyti. Prieš valant naršyklės duomenis ar perinstaliuojant PWA eksportuok vietinę atkūrimo kopiją.")}</p>{incidentId&&<code>{incidentId}</code>}<div className="button-row"><Button onClick={()=>location.reload()}><RotateCcw/>{tr("Reload app","Perkrauti programą")}</Button><Button variant="outline" onClick={()=>setDetails(!details)}><Stethoscope/>{tr("Diagnostics","Diagnostika")}</Button><Button variant="outline" onClick={()=>void exportData()}><Download/>{tr("Export local data","Eksportuoti vietinius duomenis")}</Button><Button variant="outline" onClick={()=>void send()}><Mail/>{tr("Send report","Siųsti ataskaitą")}</Button></div>{details&&<pre>{error.stack??error.message}</pre>}{message&&<p role="status">{message}</p>}<p className="field-hint">{tr("Do not uninstall the PWA or clear site data until a recovery backup exists.","Neišdiek PWA iš naujo ir nevalyk svetainės duomenų, kol neturi atkūrimo kopijos.")}</p></main>;
}
