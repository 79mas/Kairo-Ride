...
export function GoalForecasts({state,actions}:{state:State;actions?:ViewActions}){
  ...
  return <section className="goal-section panel"><div className="chart-heading"><div><p className="eyebrow">{tr("GOALS","TIKSLAI")}</p><h2><Target/>{tr("Distance goals","Ridos tikslai")}</h2></div><Button disabled={!actions} onClick={()=>open()}><Plus/>{tr("Add goal","Pridėti tikslą")}</Button></div>
    ...
    <div className="goal-grid">{goals.map(goal=>{const f=forecastGoal(state,goal),label=goalLabel(goal,state,tr),window=goalWindow(goal);return <details className="goal-card" key={goal.id}><summary><strong>{label}</strong><span>{formatKm(goal.targetKm,locale)} km</span></summary><div className="goal-card-body"><div className="section-heading"><strong>{tr("Goal details","Tikslo informacija")}</strong><Button variant="ghost" size="icon" disabled={!actions} onClick={()=>open(goal)} aria-label={tr("Edit goal","Redaguoti tikslą")+": "+label}><Pencil/></Button></div>
      ...
      <Progress value={Math.min(100,f.progressPercent)} getValueLabel={()=>formatNumber(f.progressPercent)+"%"} aria-label={label} aria-valuetext={formatNumber(f.progressPercent)+"%"}/><p>{formatKm(f.currentKm,locale)} / {formatKm(goal.targetKm,locale)} km · {formatKm(f.progressPercent,locale)}%</p>
...
export function EarthProgress({state,goalId}:{state:State;goalId?:string}){
  ...
  return <section className="earth-progress" aria-label={label}><div><span><Globe2/>{label} — {formatKm(goal.targetKm,locale)} km</span><span>{formatKm(f.currentKm,locale)} / {formatKm(goal.targetKm,locale)} km <strong>{formatNumber(f.progressPercent)}%</strong> <small className="progress-eta">({tr("Calculated achievement date","Numatoma tikslo pasiekimo data")}: {f.estimatedDate?formatDate(f.estimatedDate,false,undefined,locale):f.status==="achieved"?tr("Goal reached","Tikslas pasiektas"):tr("Not available","Nėra prognozės")})</small></span></div><Progress value={Math.min(100,f.progressPercent)} getValueLabel={()=>formatNumber(f.progressPercent)+"%"} aria-label={label}/></section>;
}
