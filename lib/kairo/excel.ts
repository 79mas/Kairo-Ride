...
export function exportWorkbook(operations:Operation[]):Workbook{
  ...
  return {
    ...
    Goals:[["ID","Wheel ID","Scope","Target km","Created (UTC)","Name","Period","Start date","End date","Archived"],...s.goal.map(goal=>[goal.id,goal.wheelId,goal.wheelId?names.get(goal.wheelId)??goal.wheelId:"All vehicles",goal.targetKm,goal.createdAt,goal.name??"",goal.period??"all",goal.startDate??"",goal.endDate??"",!!goal.archived])],
    ...
    Archived:[["Kind","ID","Archived"],...KINDS.flatMap(kind=>s[kind].filter(entity=>entity.archived).map(entity=>[kind,entity.id,true]))],
