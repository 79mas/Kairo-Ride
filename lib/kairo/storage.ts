...
function complete(tx: IDBTransaction) { return new Promise<void>((resolve,reject) => { tx.oncomplete=()=>resolve(); tx.onabort=()=>reject(tx.error ?? new Error("Saugojimas nutrauktas.")); tx.onerror=()=>reject(tx.error); }); }
function database(): Promise<IDBDatabase> {
  if (!opening) opening = new Promise((resolve,reject) => {
    ...
    req.onblocked=()=>reject(new Error("Uždaryk kitą senos versijos Kairo Ride langą ir bandyk dar kartą."));
  });
  return opening;
}
...
export async function mergeOperations(namespace: string, operations: Operation[], uploaded: boolean) {
  ...
  try {
    ...
    for (const operation of validated) {
      ...
      if (existing && canonical(existing.operation)!==canonical(operation)) throw new Error("Pakeitimų ID konfliktas.");
...
export async function addAttachment(namespace: string, ownerKind: "trip"|"ride", ownerId: string, file: File) {
  if(file.size>512*1024*1024) throw new Error("Pirmame etape vieno failo riba – 512 MB. Didesnį vaizdo įrašą kol kas laikyk atskirai Drive.");
  const estimate=await navigator.storage?.estimate?.();
  if(estimate?.quota && estimate?.usage && file.size > (estimate.quota-estimate.usage)*0.9) throw new Error("Įrenginyje nepakanka vietos šio failo kopijai. Originalas nepakeistas.");
...
export function friendlyError(error: unknown) {
  if(error instanceof DOMException && error.name === "QuotaExceededError") return "Įrenginio saugykla pilna. Šis pakeitimas neišsaugotas. Eksportuok duomenis ir atlaisvink vietos.";
  if(error instanceof Error) return error.message;
  return "Nepavyko atlikti veiksmo. Tavo ankstesni įrašai nepakeisti.";
}
