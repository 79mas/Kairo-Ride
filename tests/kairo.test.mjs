...
test("equipment metadata and concurrent revisions round-trip through actual XLSX bytes",async()=>{
  ...
  assert.equal(book.Gear[1][2],"Ryšio įranga / Cardo");
...
test("empty recovery history cannot silently import an equipment-only report as no data",async()=>{
  ...
  await assert.rejects(()=>x.workbookImport(book),/trūksta atkūrimo istorijos/);
...
test("colliding operation identifiers stop a merge",()=>{
  ...
  assert.throws(()=>d.project([a,b]),/tą patį ID/);
});
...
test("a disconnected cycle cannot hide next to an apparently valid revision",()=>{
  ...
  assert.throws(()=>d.project([a,b,c]),/ciklas/);
});
...
test("local validation rejects an impossible odometer and protects referenced wheels",()=>{
  ...
  assert.throws(()=>d.validateEdit(s,"reading",reading("new","01",200)),/sekos/);
  assert.throws(()=>d.validateDelete(s,"wheel",wheel.id),/susietų/);
});
...
test("Excel dates use selected time zone, including DST",()=>{
  ...
  assert.throws(()=>x.zonedInstant("2026-03-29T03:30:00","Europe/Vilnius"),/neegzistuoja/);
});
...
test("ambiguous old mixed-wheel sheet is explicitly rejected, not guessed",async()=>{await assert.rejects(()=>x.workbookImport({"EUC Data":[["Date","ODM"]]},"Europe/Vilnius"),/atskirai/);});
...
test("expired access tokens cannot issue requests",async()=>{let called=false;const client=new DriveClient("test-token",Date.now()-1000,async()=>{called=true;return Response.json({});});await assert.rejects(()=>client.about(),/Atnaujink/);assert.equal(called,false);});
...
test("wrong Google account stops sync before a Drive folder or event is written",async()=>{
  ...
  await assert.rejects(()=>client.sync("google:different",()=>{}),/Paskyra pasikeitė/);assert.equal(calls,1);
});
test("unsafe resumable session URLs never receive a bearer token",async()=>{
  ...
  await assert.rejects(()=>client.uploadAttachment(ns,attachment,{key:"k",namespace:ns,attachmentId:attachment.id,blob:new Blob(["abc"]),fileId:"file0000001",session:"https://attacker.invalid/upload"},"parent0001",()=>{}),/Netinkamas/);assert.equal(calls,0);
});
