...
test("equipment tab renders saved gear with category, status, details and edit controls",async()=>{
  ...
  const noop=()=>{};
  const html=renderToStaticMarkup(React.createElement(Tabs,{value:"gear"},React.createElement(MainViews,{state,actions:{openEditor:noop,askDelete:noop,setDetail:noop},setView:noop,openStorage:noop})));
  assert.match(html,/>Ekipuotė</);assert.match(html,/Kelionės Cardo/);assert.match(html,/Ryšio įranga \/ Cardo/);
  assert.match(html,/Atsarginė/);assert.match(html,/Packtalk/);assert.match(html,/Atsarginis laidas/);assert.match(html,/aria-label="Redaguoti"/);
});
